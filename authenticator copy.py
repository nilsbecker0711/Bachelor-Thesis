...
def extract_mfcc(audio_path, mfccs):
    '''
...
def train_speaker_classification(data_path, audio_path, mfcc, tune=False, save = True):
    '''
    ...
    for index, sample in voice_samples.iterrows():
        ...
        mfcc_features, sr, path = extract_mfcc(audio_path+audio_path_detail, mfcc)
    ...
    for c in cs:
        for kernel in kernels:
            ...
            for speaker_number, amount in values.items():
                ...
                while runner:
                    try: #find invalid speaker data   
                        classifier, acc, f1 = train_classifier(features, current_speaker, tune = False, kernel = kernel, c = c)
                        runner = False
                    except Exception as e:
            ...
            print(f'Config: {mfcc}, {kernel}, {c} Acc= {sum(accs)/len(accs)}, F1 = {sum(f1s)/len(f1s)}')
...
def train_classifier(features, criteria, kernel, c,  tune=False):
...
def training(save):
    ...
    mfccs = [16, 32, 64, 128, 256]
    
    for mfcc in mfccs:
        train_speaker_classification("models\svc_model24_07_2023_14_23\data.xlsx", "", tune=False, save = False, mfcc = mfcc)

def train_clone():
    train_clone_classifier("samples\\cloned\\", "", "models/svc_model24_07_2023_16_08/data.xlsx")

def test(prob):
    model_path = os.path.join(dirname, "models/svc_model24_07_2023_16_08")
    classifiers = []
    clone_clf = load_classifiers(os.path.join(dirname, "models/svc_model24_07_2023_16_17/clone.jl"))
    ...
    '''
    #(predict_single_speaker(classifiers, f"temp.wav",clone_classifier=clone_clf, proba=prob ))
    #return
